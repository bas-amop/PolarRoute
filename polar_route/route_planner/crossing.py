...
class NewtonianDistance:
    def __init__(
        ...
        self.source_speed = unit_speed(
            self.source_cellbox.agg_data["speed"][direction.index(case)],
            self.unit_shipspeed,
        )
        self.neighbour_speed = unit_speed(
            self.neighbour_cellbox.agg_data["speed"][direction.index(case)],
            self.unit_shipspeed,
        )
    ...
    def _corner(self):
        ...
        cross_points = [s_cx + ptvX * s_dcx, s_cy + ptvY * s_dcy]
        cell_points = [n_cx, n_cy]
    ...
    def value(self):
        ...
        if abs(self.case) == 2:
            ...
            logger.debug(
                "---> Issue with cell (Xsc,Ysc)={:.2f};{:.2f}"
                + format(
                    self.source_cellbox.get_bounds().getcx(),
                    self.source_cellbox.get_bounds().getcy(),
                )
            )
