...
class FindEdge:
    ...
    def __init__(self, cell_a, cell_b, case):
        self.crossing, self.case, self.start, self.end = self._find_edge(
            cell_a, cell_b, case
        )
    ...
    def _find_edge(self, cell_a, cell_b, case):
        ...
        for indx in range(len(cell_a["neighbourCrossingPoints"])):
            if (cell_a["neighbourIndex"][indx] == cell_b["id"]) and (
                cell_a["case"][indx] == case
            ):
...
class PathValues:
    ...
    def _segment_costs(self, path_requested_variables, source_graph, wp, cp):
        ...
        ssp = unit_speed(
            source_graph["speed"][self.direction.index(case)], self.unit_shipspeed
        )
        ...
        for var in path_requested_variables.keys():
            if var == "distance":
                ...
                if var in source_graph.keys():
                    ...
                    if isinstance(source_graph[var], list):
    ...
    def objective_function(self, adjacent_pairs, start_waypoint, end_waypoint):
        ...
        for ii in range(len(adjacent_pairs) + 1):
            ...
            segment_variable, segment_case = self._segment_costs(
                self.path_requested_variables, cellbox, wp, cp
            )

            # Adding that value for the segment along the paths
            for var in segment_variable:
                if isinstance(segment_variable[var], np.ndarray):
                    variables[var]["path_values"][ii + 1] = segment_variable[var][
                        self.direction.index(segment_case)
                    ]
                else:
                    variables[var]["path_values"][ii + 1] = segment_variable[var]
...
class Smoothing:
    def __init__(
        ...
        for key in self.dijkstra_graph.keys():
            ...
            if len(cell["neighbourTravelLegs"]) > 0:
                accessible_edges = np.where(
                    np.isfinite(np.sum(cell["neighbourTravelLegs"], axis=1))
                )[0]
                cell["case"] = cell["case"][accessible_edges]
                cell["neighbourIndex"] = cell["neighbourIndex"][accessible_edges]
                cell["neighbourCrossingPoints"] = cell["neighbourCrossingPoints"][
                    accessible_edges
                ]
                cell["neighbourTravelLegs"] = cell["neighbourTravelLegs"][
                    accessible_edges
                ]
    ...
    def _long_case(self, start, end, case, Sp, Cp, Np):
        ...
        def newton_optimisation_long(
            f, y0, x, a, Y, u1, v1, u2, v2, speed_s, speed_e, R, λ_s, φ_r
        ):
            """
            ...
            while improving:
                F, dF, X1, X2 = f(
                    y0, x, a, Y, u1, v1, u2, v2, speed_s, speed_e, R, λ_s, φ_r
                )
                if (F == 0) or (dF == 0):
                    dY = 0
                else:
                    dY = F / dF
                if iter_number != 0:
                    improving = (abs(dY) > _epsilon) or (
                        abs(dY) > _epsilon * (X1 * X2)
        ...
        Y = (Np[1] - Sp[1]) * 111.386 * 1000.0
        y0 = Y / 2
        u1 = sgn * cell_s_u
        v1 = cell_s_v
        u2 = sgn * cell_e_u
        v2 = cell_e_v
        y = newton_optimisation_long(
            _F, y0, x, a, Y, u1, v1, u2, v2, speed_s, speed_e, Rd, λ_s, φ_r
        )

        # Updating the crossing points
        Cp = (Cp[0], Sp[1] + y / (111.386 * 1000.0))
    ...
    def _lat_case(self, start, end, case, Sp, Cp, Np):
        ...
        def newton_optimisation_lat(
            f, y0, x, a, Y, u1, v1, u2, v2, speed_s, speed_e, R, λ, θ, ψ
        ):
            """
            ...
            while improving:
                F, dF, X1, X2 = f(
                    y0, x, a, Y, u1, v1, u2, v2, speed_s, speed_e, R, λ, θ, ψ
                )
                if (F == 0) or (dF == 0):
                    dY = 0
                else:
    ...
    def newton_smooth(self, start, end, case, firstpoint, midpoint, lastpoint):
        ...
        """
        if abs(case) == 2:
            midpoint = self._long_case(
                start, end, case, firstpoint, midpoint, lastpoint
            )
        elif abs(case) == 4:
    ...
    def _neighbour_case(self, cell_a, cell_b, x, case):
        ...
        raise RouteSmoothingError(
            "Path Smoothing - Failure - Adding additional cases unknown in neighbour_case"
        )
    ...
    def _neighbour_indices(self, cell_a, cell_b, case, add_case_a, add_case_b):
        ...
        for cell_a_neighbour in cell_a_neighbours:
            ...
            _connections = set(_possible_cell["neighbourIndex"]).intersection(
                cell_b_neighbours
            )
    ...
    def _neighbour_cells(self, cell_a, cell_b, case, add_case_a, add_case_b):
        ...
        Returns
            ...
            additional_cases (list) - A list of the cases connecting the additional cell indices. None if no index added.
        """
        add_indices, add_cases = self._neighbour_indices(
            cell_a, cell_b, case, add_case_a, add_case_b
        )
    ...
    def nearest_neighbour(self, start, end, case, x):
        ...
        add_indices, add_edges = self._neighbour_cells(
            start, end, case, target_a_case, target_b_case
        )
    ...
    def blocked(self, new_cell, cell_a, cell_b):
        ...
        if self.blocked_metric == "objective":
            ...
            return self.blocked_objective(
                new_cell, cell_a, cell_b, blocked_variable=blocked_var
            )
        # Otherwise block based on metric specified
        elif self.blocked_metric == "speed":
            return self.blocked_objective(
                new_cell, cell_a, cell_b, blocked_variable="speed"
            )
        elif self.blocked_metric == "traveltime":
            return self.blocked_tt(new_cell, cell_a, cell_b)
        elif self.blocked_metric == "fuel":
            return self.blocked_objective(
                new_cell, cell_a, cell_b, blocked_variable="fuel"
            )
        elif self.blocked_metric == "battery":
            return self.blocked_objective(
                new_cell, cell_a, cell_b, blocked_variable="battery"
            )
    ...
    def blocked_objective(self, new_cell, cell_a, cell_b, blocked_variable):
        ...
        objective_diff_1 = (
            new_objective_start - old_objective_start
        ) / old_objective_start
        ...
        if blocked_variable == "speed":
            if objective_diff_1 < -self.blocking_percentage / 100.0:
                return True
            elif objective_diff_2 < -self.blocking_percentage / 100.0:
                return True
            else:
                return False
        else:
    ...
    def blocked_tt(self, new_cell, cell_a, cell_b):
        ...
        tt_diff = ((new_tt_start + new_tt_end) - (old_tt_start + old_tt_end)) / (
            old_tt_start + old_tt_end
        )

        if tt_diff > self.blocking_percentage / 100.0:
            return True
        else:
            return False
    ...
    def blocked_ice(self, new_cell, cell_a, cell_b):
        ...
        if (
            (percentage_diff1 <= self.blocking_percentage * start)
            or (percentage_diff2 <= self.blocking_percentage * end)
    ...
    def diagonal_select_side(
        self, cell_a, cell_b, case, firstpoint, midpoint, lastpoint
    ):
        """
        ...
        add_indices, add_edges = self._neighbour_cells(
            cell_a, cell_b, case, target_a_case, target_b_case
        )
    ...
    def dist(self, start_point, end_point):
        ...
        distance = np.sqrt(
            ((sp_lon - ep_lon) * 111.321) ** 2 + ((sp_lat - ep_lat) * 111.386) ** 2
        )
        ...
        while not converged:
            ...
            while ii < path_length:
                ...
                if self.dist(midpoint, lastpoint) < self.merge_separation:
                    ...
                    common_cell = np.where(
                        np.array(start_cell["neighbourIndex"]) == end_cell["id"]
                    )[0]
                    if len(common_cell) == 1:
                        _merge_case = start_cell["case"][
                            np.where(
                                np.array(start_cell["neighbourIndex"]) == end_cell["id"]
                            )[0][0]
                        ]
                ...
                if self.diagonal_case(ap.case):
                    add_indices, add_cases = self.diagonal_select_side(
                        ap.start, ap.end, ap.case, firstpoint, midpoint, lastpoint
                    )
                    if add_indices is None:
                        ii += 1
                        firstpoint = midpoint
                    ...
                    if len(add_indices) == 1:
                        ...
                        if self.blocked(target, ap.start, ap.end):
                            ...
                            if self.previous_diagonals(
                                edge_a, edge_b, firstpoint, lastpoint
                            ):
                                ii += 1
                                firstpoint = midpoint
                                continue
                ...
                midpoint_prime = self.newton_smooth(
                    ap.start, ap.end, ap.case, firstpoint, midpoint, lastpoint
                )
                if (
                ...
                add_indices, add_cases = self.nearest_neighbour(
                    ap.start, ap.end, ap.case, midpoint_prime
                )

                # No additional cells to add
                if add_indices is None:
                    midpoint_prime = self.clip(
                        ap.start, ap.end, ap.case, midpoint_prime
                    )
                ...
                if len(add_indices) == 2:
                    ...
                    if not self.blocked(
                        target_a, ap.start, ap.end
                    ) and not self.blocked(target_b, ap.start, ap.end):
