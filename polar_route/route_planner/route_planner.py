...
from polar_route.route_planner.crossing_smoothing import (
    Smoothing,
    FindEdge,
    PathValues,
    rhumb_line_distance,
...
from polar_route.utils import (
    json_str,
    unit_speed,
    pandas_dataframe_str,
    case_from_angle,
    timed_call,
)
from polar_route.exceptions import NoRouteFoundError, InvalidMeshError
...
def _mesh_boundary_polygon(mesh):
    ...
    if (
        mesh["config"]["mesh_info"]["region"]["long_min"] == -180
        or mesh["config"]["mesh_info"]["region"]["long_max"] == 180
    ):
        tiny_value = 0
    else:
        tiny_value = 1e-10
...
def _adjust_waypoints(point, cellboxes, max_distance=5):
    ...
    if point.within(cb_polygon):
        ...
        if distance_away > max_distance:
            logger.info("Waypoint too far from accessible cellbox!")
            return point
...
def initialise_dijkstra_route(dijkstra_graph, dijkstra_route):
    ...
    points = np.concatenate(
        [
            org_path_points[0, :][None, :],
            org_path_points[1:-1:2],
            org_path_points[-1, :][None, :],
        ]
    )
    cell_indices = np.concatenate(
        [
            [org_cell_indices[0]],
            [org_cell_indices[0]],
            org_cell_indices[1:-1:2],
            [org_cell_indices[-1]],
            [org_cell_indices[-1]],
        ]
    )
    cell_cases = np.concatenate(
        [
            [org_cell_cases[0]],
            [org_cell_cases[0]],
            org_cell_cases[1:-1:2],
            [org_cell_cases[-1]],
            [org_cell_cases[-1]],
        ]
    )
...
def _load_waypoints(waypoints):
    ...
    try:
        ...
        src_wps = [
            Waypoint(lat=source["Lat"], long=source["Long"], name=source["Name"])
            for index, source in source_waypoints_df.iterrows()
        ]
        dest_wps = [
            Waypoint(lat=dest["Lat"], long=dest["Long"], name=dest["Name"])
            for index, dest in dest_waypoints_df.iterrows()
        ]
        return src_wps, dest_wps
    except FileNotFoundError:
        raise ValueError(
            f"Unable to load '{waypoints}', please check file path provided"
        )
    ...
    **compute_smoothed_routes**: Smooths the output of **compute_routes** using information from the environmental mesh to determine mesh independent optimal routes.
    ...
    def __init__(self, mesh_file, config_file, cost_func=NewtonianDistance):
        ...
        self.cellboxes_lookup = {
            str(self.env_mesh.agg_cellboxes[i].get_id()): self.env_mesh.agg_cellboxes[i]
        ...
        if all(cb.agg_data["inaccessible"] for cb in self.cellboxes_lookup.values()):
            raise InvalidMeshError(
                "The environment mesh contains no accessible cells, routing is impossible!"
            )
        ...
        for name in self.vector_names:
            if not any(name in cb.agg_data for cb in self.cellboxes_lookup.values()):
                raise InvalidMeshError(
                    f"The environment mesh does not contain {name} data and it is a prerequisite for the "
                    f"route planner!"
                )
        ...
        if not any("speed" in cb.agg_data for cb in self.cellboxes_lookup.values()):
            raise InvalidMeshError(
                "Vessel speed not in the mesh information! Please run vessel performance"
            )

        # Check if objective function is in the environment mesh (e.g. speed)
        if self.config["objective_function"] != "traveltime":
            if not any(
                self.config["objective_function"] in cb.agg_data
                for cb in self.cellboxes_lookup.values()
            ):
                raise InvalidMeshError(
                    f"Objective Function '{self.config['objective_function']}' requires the mesh cellboxes"
    ...
    def _splitting_around_waypoints(self, waypoints_df):
        ...
        """
        if ("waypoint_splitting" in self.config) and (
            self.config["waypoint_splitting"]
        ):
            logger.info(" Splitting around waypoints !")
            prior_cells = [
                cellbox.get_id()
                for cellbox in self.env_mesh.agg_cellboxes
                if not cellbox.agg_data["inaccessible"]
            ]
            wps_points = [
                (entry["Lat"], entry["Long"]) for _, entry in waypoints_df.iterrows()
            ]
            ...
            for e_wp in end_waypoints:
                ...
                if s_wp.equals(e_wp):
                    ...
                    if s_wp.get_name() != e_wp.get_name():
                        logger.info(
                            f"Route from {s_wp.get_name()} to {e_wp.get_name()} not calculated, these waypoints"
                            f" are identical"
                        )
                    continue
                route_segments = []
                ...
                if s_wp.get_cellbox_indx() == e_wp_indx:
                    ...
                    while s_wp.get_cellbox_indx() != e_wp_indx:
                        ...
                        if routing_info.get_node_index() == -1:
                            logger.warning(
                                f"{s_wp.get_name()} to {e_wp.get_name()} - Failed to construct Dijkstra route"
                            )
                            no_route_found = True
                            break
                        ...
                        neighbour_case = (
                            self.env_mesh.neighbour_graph.get_neighbour_case(
                        ...
                        logger.debug(
                            f"route segments >> {route_segments[0][0].to_str()}"
                        )

                    route_segments = list(itertools.chain.from_iterable(route_segments))
                    route = Route(
                        route_segments, s_wp.get_name(), e_wp.get_name(), self.config
                    )
                    route.source_waypoint = s_wp
                ...
                route.waypoint_correction(
                    self.cellboxes_lookup[
                        route.segments[0].get_start_wp().get_cellbox_indx()
                    ],
                    s_wp,
                    route.segments[0].get_end_wp(),
                    0,
                )
                ...
                if len(route.segments) > 1:
                    route.waypoint_correction(
                        self.cellboxes_lookup[
                            route.segments[-1].get_end_wp().get_cellbox_indx()
                        ],
                        e_wp,
                        route.segments[-1].get_start_wp(),
                        -1,
                    )
                routes.append(route)
    ...
    def _neighbour_cost(self, node_id, neighbour_id, case):
        ...
        if not math.isnan(traveltime[0]) and not math.isnan(traveltime[1]):
            self.neighbour_legs[node_id + "to" + neighbour_id] = (
                traveltime,
                crossing_points,
            )
        else:
        ...
        s1 = Segment(
            Waypoint.load_from_cellbox(self.cellboxes_lookup[node_id]),
            Waypoint(crossing_points[1], crossing_points[0], cellbox_indx=node_id),
        )
        s2 = Segment(
            Waypoint(crossing_points[1], crossing_points[0], cellbox_indx=neighbour_id),
        ...
        if "fuel" in self.config["path_variables"]:
            s1.set_fuel(
                s1.get_travel_time()
                * self.cellboxes_lookup[node_id].agg_data["fuel"][
                    self.direction.index(case)
                ]
            )
        if "battery" in self.config["path_variables"]:
            s1.set_battery(
                s1.get_travel_time()
                * self.cellboxes_lookup[node_id].agg_data["battery"][
                    self.direction.index(case)
                ]
            )
        s1.set_distance(
            s1.get_travel_time()
            * unit_speed(
                self.cellboxes_lookup[node_id].agg_data["speed"][
                    self.direction.index(case)
                ],
        ...
        if "fuel" in self.config["path_variables"]:
            s2.set_fuel(
                s2.get_travel_time()
                * self.cellboxes_lookup[neighbour_id].agg_data["fuel"][
                    self.direction.index(case)
                ]
            )
        if "battery" in self.config["path_variables"]:
        ...
        s2.set_distance(
            s2.get_travel_time()
            * unit_speed(
                self.cellboxes_lookup[neighbour_id].agg_data["speed"][
                    self.direction.index(case)
                ],
                self.config["unit_shipspeed"],
            )
    ...
    def compute_routes(self, waypoints):
        ...
        waypoints_df["Source"] = [
            s.strip().upper() if isinstance(s, str) else np.nan
            for s in waypoints_df["Source"]
        ]
        waypoints_df["Destination"] = [
            s.strip().upper() if isinstance(s, str) else np.nan
            for s in waypoints_df["Destination"]
        ]
        ...
        if not src_wps:
            raise NoRouteFoundError(
                "Invalid waypoints. No accessible source waypoints specified"
            )
        if not end_wps:
            raise NoRouteFoundError(
                "Invalid waypoints. No accessible destination waypoints specified"
            )
    ...
    def compute_smoothed_routes(self):
        ...
        if not self.routes_dijkstra:
            raise NoRouteFoundError(
                "No Dijkstra routes found when attempting to smooth routes"
            )
        ...
        for route in routes:
            ...
            if len(route_json["properties"]["CellIndices"]) == 1:
                logger.info(
                    f"--- Skipping smoothing for {route_json['properties']['name']}, direct route within a"
                    f" single cell"
                )
                ...
                route_json["properties"]["distance"] = [
                    0.0,
                    rhumb_line_distance(start_location, end_location),
                ]
                route_json["properties"]["speed"] = [
                    0.0,
                    self.cellboxes_lookup[route_cell].agg_data["speed"][case_idx],
                ]
                route_json["properties"]["route_type"] = "smoothed"

                for var in self.config["path_variables"]:
                    route_json["properties"][var].insert(0, 0.0)
            ...
            initialised_dijkstra_graph = self.initialise_dijkstra_graph(
                cellboxes, neighbour_graph, route
            )
            adjacent_pairs, source_wp, end_wp = initialise_dijkstra_route(
                initialised_dijkstra_graph, route_json
            )
            ...
            sf = Smoothing(
                ...
                blocking_percentage=blocked_percentage,
            ...
            path_info = pv.objective_function(
                sf.aps, sf.start_waypoint, sf.end_waypoint
            )
            ...
            smoothed_route["properties"]["total_traveltime"] = smoothed_route[
                "properties"
            ]["traveltime"][-1]
            ...
            if "fuel" in self.config["path_variables"]:
                ...
                smoothed_route["properties"]["total_fuel"] = smoothed_route[
                    "properties"
                ]["fuel"][-1]
            if "battery" in self.config["path_variables"]:
                battery_legs = variables["battery"]["path_values"]
                smoothed_route["properties"]["battery"] = list(battery_legs)
                smoothed_route["properties"]["total_battery"] = smoothed_route[
                    "properties"
                ]["battery"][-1]
    ...
    def initialise_dijkstra_graph(
        self, cellboxes, neighbour_graph, route, path_index=False
    ):
        """
        ...
        for cell in cellboxes:
            ...
            dijkstra_graph_dict[cell_id]["Vector_x"] = dijkstra_graph_dict[cell_id][
                self.config["vector_names"][0]
            ]
            dijkstra_graph_dict[cell_id]["Vector_y"] = dijkstra_graph_dict[cell_id][
                self.config["vector_names"][1]
            ]
            ...
            dijkstra_graph_dict[cell_id]["neighbourTravelLegs"] = np.array(
                neighbour_travel_legs
            )
            dijkstra_graph_dict[cell_id]["neighbourCrossingPoints"] = np.array(
                neighbour_crossing_points
            )
            if path_index:
                dijkstra_graph_dict[cell_id]["pathIndex"] = (
                    route.source_waypoint.get_path_nodes(str(cell_id))
                )
    ...
    def _validate_wps(self, wps):
        ...
        def select_cellbox(ids):
            ...
            if self.env_mesh.neighbour_graph.get_neighbour_case(
                self.cellboxes_lookup[ids[0]], self.cellboxes_lookup[ids[1]]
            ) in [Direction.east, Direction.north_east, Direction.north]:
                return ids[1]
            return ids[0]
    ...
    def to_json(self):
        ...
        if self.routes_smoothed:
            ...
            output_json["paths"]["features"] = [
                dr.to_json(route_type="dijkstra") for dr in self.routes_dijkstra
            ]
        else:
            output_json["paths"] = []
